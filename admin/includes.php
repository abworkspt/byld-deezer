...
function include_scripts_styles() {
    if(!is_admin() && !in_array( $_SERVER['PHP_SELF'], array( '/wp-login.php', '/wp-register.php' ) )) {
		...
		wp_enqueue_script('main', get_template_directory_uri().'/scripts/main-min.js', '', '1.5', true);
		...
		// TEMPLATE
		wp_enqueue_style( 'main',  get_template_directory_uri(). '/css/main.min.css',false,'2.2','all');
    }
}
